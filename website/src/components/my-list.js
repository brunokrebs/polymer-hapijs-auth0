    ...
    <style>
      ...
      <ul>
        ${properties.items.map((item, index) => html`
          <li>${item.name} <span hidden?="${!( item.loading || item.deleting)}" class="spinner"></span> <span hidden?="${item.deleting || item.loading}" on-click="${() => this._deleteEntry(index)}">(Delete)</span></li>
        `)}
      </ul>
